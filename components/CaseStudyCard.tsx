...
export default function CaseStudyCard({ card, width }: CaseStudyCardProps) {
  return (
    <article
      className="border border-zinc-400/16 rounded-card shrink-0 overflow-hidden"
    ...
    >
      ...
      >
        {/* Media — top under lg, right at lg+. Always 16:9 so height tracks card width. */}
        <div className="order-1 lg:order-2 lg:flex-1 lg:min-w-0 p-3 lg:pl-0 lg:flex lg:items-start">
          <div className="relative w-full aspect-[16/9] rounded-xl border border-black/10 overflow-hidden">
            <Image
              alt={card.title}
              src={card.image}
              fill
        ...
        <div className="order-2 lg:order-1 lg:w-100 lg:shrink-0 p-card flex flex-col justify-between gap-block lg:self-stretch">
